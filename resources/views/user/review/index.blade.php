        ...
        <table class="table table-bordered" id="order-dataTable" width="100%" cellspacing="0">
          ...
          <tbody>
            @foreach($reviews as $review)  
              @php 
              $nom=DB::table('materiels')->select('nom')->where('id',$review->materiel_id)->get();
              @endphp
                <tr>
                    <td>{{$review->id}}</td>
                    <td>{{$review->user_info['name']}}</td>
                    <td>@foreach($nomtitle as $data){{ $data->nom}} @endforeach</td>
                    <td>{{$review->review}}</td>
                    <td>
                     <ul style="list-style:none">
                          @for($i=1; $i<=5;$i++)
                          @if($review->rate >=$i)
                            <li style="float:left;color:#4caf50;"><i class="fa fa-star"></i></li>
                          @else 
                            <li style="float:left;color:#4caf50;"><i class="far fa-star"></i></li>
                          @endif
                        @endfor
                     </ul>
                    </td>
                    ...
                    <td>
                        <a href="{{route('user.materielreview.edit',$review->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                        <form method="POST" action="{{route('user.materielreview.delete',[$review->id])}}">
                          @csrf 
                          @method('delete')
