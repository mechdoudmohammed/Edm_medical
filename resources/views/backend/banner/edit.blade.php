...
@section('title','EDM-Medical || Banner Edit')
@section('main-content')

<div class="card">
    <h5 class="card-header">Modifier bannière</h5>
    <div class="card-body">
      <form method="post" action="{{route('banner.update',$banner->id)}}">
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            <option value="active" {{(($banner->status=='active') ? 'selected' : '')}}>Activer</option>
            <option value="inactive" {{(($banner->status=='inactive') ? 'selected' : '')}}>Désactiver</option>
          </select>
          @error('statut')
          <span class="text-danger">{{$message}}</span>
          @enderror
